...
func mark_granted() -> void:
	if !animationRunning:
		...
		achievementTween.tween_callback(achievementTween.kill)
...
func update_process(percent: float) -> void:
	...
	var achievementTween: Tween = self.create_tween()
	achievementTween.tween_property(self, "scale", Vector2(1.2, 1.2), 0.2)
	achievementTween.tween_property(self, "scale", Vector2(1.0, 1.0), 0.2)
	achievementTween.tween_callback(achievementTween.kill)
